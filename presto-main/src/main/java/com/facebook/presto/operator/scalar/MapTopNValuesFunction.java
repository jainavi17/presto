...
public final class MapTopNValuesFunction
{
    ...
    public static Block computeTopNBlock(Type type, Block sortedBlock, long n)
    {
        ...
        long cnt = 0;
        for (int i = arrayLength - 1; i >= 0 && cnt < Math.abs(n); --i) {
            int position = (n < 0) ? arrayLength - 1 - i : i;

            if (sortedBlock.isNull(position)) {
                continue;
            }
            type.appendTo(sortedBlock, position, blockBuilder);
            cnt++;
        }
